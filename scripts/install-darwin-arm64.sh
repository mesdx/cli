#!/usr/bin/env bash
# Install MesDX for macOS (Apple Silicon)
...
echo "📦 Installing MesDX (macOS Apple Silicon)..."
echo ""
...
curl -L "${BASE_URL}/mesdx-darwin-arm64" -o "${BIN_DIR}/mesdx"
chmod +x "${BIN_DIR}/mesdx"

echo "⬇️  Downloading parsers..."
curl -L "${BASE_URL}/mesdx-parsers-darwin-arm64.tar.gz" | tar xz -C "${LIB_DIR}" --strip-components=1
