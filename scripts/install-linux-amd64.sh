...
echo "📦 Installing MesDX (Linux AMD64)..."
echo ""
...
curl -L "${BASE_URL}/mesdx-linux-amd64" -o "${BIN_DIR}/mesdx"
chmod +x "${BIN_DIR}/mesdx"

echo "⬇️  Downloading parsers..."
curl -L "${BASE_URL}/mesdx-parsers-linux-amd64.tar.gz" | tar xz -C "${LIB_DIR}" --strip-components=1
