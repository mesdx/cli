...
echo "📦 Installing MesDX (macOS Intel)..."
echo ""
...
curl -L "${BASE_URL}/mesdx-darwin-amd64" -o "${BIN_DIR}/mesdx"
chmod +x "${BIN_DIR}/mesdx"

echo "⬇️  Downloading parsers..."
curl -L "${BASE_URL}/mesdx-parsers-darwin-amd64.tar.gz" | tar xz -C "${LIB_DIR}" --strip-components=1
